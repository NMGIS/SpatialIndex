-- Run this in the Supabase SQL Editor to update the query_combined_bbox function.
...
DROP FUNCTION IF EXISTS query_combined_bbox(float, float, float, float);

CREATE FUNCTION query_combined_bbox(min_lng float, min_lat float, max_lng float, max_lat float)
